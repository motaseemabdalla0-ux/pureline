...
def ops_dashboard(db: Session = Depends(get_db)):
    ...
    # Recent activity: merge the last 15 operation log entries, pest
    # detections, and irrigation events by time, for an activity timeline.
    op_logs = (
        db.query(models.OperationLogEntry)
        .order_by(models.OperationLogEntry.created_at.desc())
        .limit(15)
        .all()
    )
    op_events = [
        {
            "kind": "operation_status", "reference": ol.operation.operation_id if ol.operation else None,
            "status": ol.status.value if hasattr(ol.status, "value") else ol.status,
            "note": ol.note, "timestamp": ol.created_at,
        }
        for ol in op_logs
    ]
    ...
    pest_detections = (
        ...
        .order_by(models.PestDetection.detected_date.desc())
        .limit(15)
        .all()
    )
    pest_events = [
        {
            "kind": "pest_detection", "reference": pd.detection_id, "status": pd.status.value,
            "note": f"{pd.risk_level.value} risk detection on {pd.farm_code}", "timestamp": pd.detected_date,
        }
    ...
    irrigation_events = (
        ...
        .order_by(models.IrrigationEvent.created_at.desc())
        .limit(15)
        .all()
    )
    irrigation_activity = [
        {
            "kind": "irrigation_event", "reference": str(ie.id),
            "status": ie.status.value if hasattr(ie.status, "value") else ie.status,
            "note": f"Zone {ie.zone_id} irrigation event", "timestamp": ie.created_at,
        }
        for ie in irrigation_events
    ]

    recent_activity = sorted(
        op_events + pest_events + irrigation_activity,
    ...
    return {
        ...
        "completed_operations_this_month": completed_this_month,
